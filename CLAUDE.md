...
- Projects are buckets. Time is linked to a project via **`day_assignments`**, keyed `(date, app_key, title)`. This is **per-day on purpose**: dragging an app/title onto a project in the UI assigns only the selected day(s), not all history.
- `title = ""` is the **app-level** assignment: a fallback covering every title not assigned on its own. The resolution rule lives in one place — the private **`Assignments`** module in `db.rs`: `link(date, key, title)` is the exact link (what the day view shows per row, so inheritance renders) and `resolve(date, key, title)` adds the app-level fallback (what `project_breakdown` / `project_apps` bill each segment to). Don't re-open-code this fallback at call sites.
